...
int dp[32][62][402][602];

// begin 表示起点，end表示终点， village表示村庄，mine 表示矿山
int day[32], end, begin, village, mine;

// 找到最优解的方案
void dfs(int time, int now, int wt, int fd) {
    if (time < 0) return;
    cout << "时间 = " << time << " ，到达地点 = " << now << "，剩余多少水 =  " << wt<< " ，剩余多少食物 = " << fd << "，剩余金钱 = " << dp[time][now][wt][fd] << endl; 
    if (now == village) {
        for (int i = 0; i <= 400; i++) {
            for (int j = 0; j <= 600; j++) {
...
int main() {
    ...
    */
    ...
    Map[1].push_back(25), Map[25].push_back(1);
    Map[2].push_back(3), Map[3].push_back(2);
    Map[3].push_back(4), Map[4].push_back(3);
    Map[3].push_back(25), Map[25].push_back(3);
    Map[4].push_back(5), Map[5].push_back(4);
    Map[4].push_back(24), Map[24].push_back(4);
    Map[4].push_back(25), Map[25].push_back(4);
    Map[5].push_back(6), Map[6].push_back(5);
    Map[5].push_back(24), Map[24].push_back(5);
    Map[6].push_back(7), Map[7].push_back(6);
    Map[6].push_back(23), Map[23].push_back(6);
    Map[6].push_back(24), Map[24].push_back(6);
    Map[7].push_back(8), Map[8].push_back(7);
    Map[7].push_back(22), Map[22].push_back(7);
    Map[8].push_back(9), Map[9].push_back(8);
    Map[8].push_back(22), Map[22].push_back(8);
    Map[9].push_back(10), Map[10].push_back(9);
    Map[9].push_back(15), Map[15].push_back(9);
    Map[9].push_back(16), Map[16].push_back(9);
    Map[9].push_back(17), Map[17].push_back(9);
    Map[9].push_back(21), Map[21].push_back(9);
    Map[9].push_back(22), Map[22].push_back(9);
    Map[10].push_back(11), Map[11].push_back(10);
    Map[10].push_back(13), Map[13].push_back(10);
    Map[10].push_back(15), Map[15].push_back(10);
    Map[11].push_back(12), Map[12].push_back(11);
    Map[11].push_back(13), Map[13].push_back(11);
    Map[12].push_back(13), Map[13].push_back(12);
    Map[12].push_back(14), Map[14].push_back(12);
    Map[13].push_back(14), Map[14].push_back(13);
    Map[13].push_back(15), Map[15].push_back(13);
    Map[14].push_back(15), Map[15].push_back(14);
    Map[14].push_back(16), Map[16].push_back(14);
    Map[15].push_back(16), Map[16].push_back(15);
    Map[16].push_back(17), Map[17].push_back(16);
    Map[16].push_back(18), Map[18].push_back(16);
    Map[17].push_back(18), Map[18].push_back(17);
    Map[17].push_back(21), Map[21].push_back(17);
    Map[18].push_back(19), Map[19].push_back(18);
    Map[19].push_back(20), Map[20].push_back(19);
    Map[20].push_back(21), Map[21].push_back(22);
    Map[21].push_back(22), Map[22].push_back(21);
    Map[21].push_back(23), Map[23].push_back(21);
    Map[21].push_back(27), Map[27].push_back(21);
    Map[22].push_back(23), Map[23].push_back(22);
    Map[23].push_back(24), Map[24].push_back(23);
    Map[23].push_back(26), Map[26].push_back(23);
    Map[24].push_back(25), Map[25].push_back(24);
    Map[24].push_back(26), Map[26].push_back(24);
    ...
    for (int time = 1; time <= 30; time++) {
        for (int node = 2; node <= 27; node++) {
            if (node == village) {              // 到了村庄
                  for (int to: Map[node]) {
                        for (int water = 0; water <= 400; water++) {
                            for (int food = 0; food <= 600; food++) {
                                if (day[time] == 1) {
    ...
    for (int i = 1; i <= 30; i++) {
        for(int water = 0; water <= 400; water++) {
            for (int food = 0; food <= 600; food++) {
                if (water * 3 + food * 2 <= 1200) {
                    ans = max(ans, dp[i][27][water][food]);
                }
            }
        }
    }
     ...
     for (int i = 1; i <= 30; i++) {
        for(int water = 0; water <= 400; water++) {
            for (int food = 0; food <= 600; food++) {
                if (water * 3 + food * 2 <= 1200) {
                    if (ans == dp[i][27][water][food]) {
                        dfs(i, 27, water, food);
                        cout << "最终剩余金额：" << ans << endl;
                        return 0;
                    }
                }
