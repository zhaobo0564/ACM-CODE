...
int main() {
    ...
    for (int i = 1; i <= q; i++) {
       ...
       // cout << "deep " << deep[s] << " t = " << deep[s] - 2 * deep[lca] << " lca " << lca << endl;
        update(deep[s] + n, 1, 1, 3 * n, rt[s]);
        update(deep[s] + n, -1, 1, 3 * n, rt[fa[lca][0]]);
        update(deep[s] - 2 * deep[lca] + n, 1, 1, 3 * n, rt[t]);
        update(deep[s] - 2 * deep[lca] + n, -1, 1, 3 * n, rt[lca]);
    }
    work(1, 0);
    for (int i = 1; i <= n; i++) {
        printf("%d ", ans[i]);
